...
@company_router.get("/companies/", operation_id="companies",)
...
@company_router.get("/company_get_by_id/{company_id}", operation_id="company_get_by_id")
async def company_get_by_id(company_id: str, session: AsyncSession = Depends(get_db)):
    company_repo = CompanyService(session)
    return await company_repo.get_by_id(company_id)


@company_router.put("/company_update/{company_id}", response_model=CompanyUpdate, operation_id="company_update")
...
@company_router.put("/company_delete/{company_id}", status_code=HTTPStatus.NO_CONTENT, operation_id="company_delete")
